...
def main(
    ...
    @functools.partial(shard_map.shard_map, mesh=mesh, in_specs=(None, pspec, pspec,pspec), out_specs=pspec)
    @functools.partial(jax.vmap, in_axes=(None, 0, 0, 0))
    def _eval(config: EvalConfig, rng: jax.Array, level: kenv_state.EnvState, state_dict):
        policy = _model.FlowPolicy(
            obs_dim=obs_dim,
        ...
        eval_info, _ = eval(config, env, rng, level, policy, env_params, static_env_params)
        return eval_info
